...
* **interval**: Identifier for the 5-minute interval in which measurement was taken,
* **average_steps**: The average number of steps recorded for the interval.

```{r average_steps_per_day}
data.avarages <- data[,mean(steps, na.rm=TRUE),by=interval]
setnames(data.avarages, c("interval", "steps"))
```
...
plot.avg_steps <- ggplot(data.avarages, aes(x=interval, y=steps))
...
The peak activity of **206.1698** steps in avarage was recorded in 5-minute interval identified 
as **835**. 

```{r max_avarage_steps}
maximum_steps <- max(data.avarages$steps)
data.avarages[data.avarages$steps == maximum_steps]
```
